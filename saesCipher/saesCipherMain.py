    ...
    if not os.path.exists(os.path.join('keysFrequency.json')):
        ...
        for i in range(0x0000, 0x10000):
            ...
            for key, value in generatedKeys.items():
                keysFrequency[key].update([value.uint])
...
def checkAllCombinations(args, cipher: saesCipher):
    ...
    if args.savePathAllCombinations:
        foldePath = os.path.join(actual_folder, args.savePathAllCombinations)
    else:
        foldePath = os.path.join(actual_folder, 'results', 'saes')
    
    checkPath(foldePath)

    filePath = os.path.join(foldePath, 'keys.csv')
