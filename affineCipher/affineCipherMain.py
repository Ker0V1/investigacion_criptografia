...
def getConsoleArguments() -> argparse.Namespace:
    ...
    parser.add_argument('--CheckAllCombinations',
                        action='store_true', default=False, help='Check all possible combinations for decryption.')
    parser.add_argument('--savePathAllCombinations', type=str,
                        help='The file path where the output of checking all combinations should be saved (default: results/affineCipher/Shifts.csv)')
...
if __name__ == "__main__":
    ...
    if args.CheckAllCombinations:
        ...
        if args.savePathAllCombinations:
            ...
            folderPath = os.path.join(
                actual_folder, 'results', 'affineCipher')

        checkPath(folderPath)
        
        filePath = os.path.join(folderPath, 'Shifts.csv')
